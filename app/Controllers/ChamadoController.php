...
class ChamadoController extends Controller
{
    ...
    public function show($request)
    {
        ...
        // VERIFICAR SE CHAMADO PODE SER INICIADO TRAJETO
        $chamado->iniciar_trajeto = false;
        ...
        // VERIFICAR SE O CHAMADO ESTÁ EM UM STATUS QUE PODE SER INICIADO
        if (in_array($chamado->status_id, [1, 6, 7, 10])) {

            // VERIFICAR SE O TÉCNICO JÁ NÃO ESTÁ EM UM CHAMADO EM TRANSITO OU EM ATENDIMENTO
            $chamado_transito = Chamado::whereIn([2, 5], "status_id")
                ->where("tecnico_id", $this->user->tecnico_id)
                ->total();

            if ($chamado_transito==0) {

                // VERIFICAR SE EXISTE ALGUM TRAJETO AVULSO ABERTO
                // $rota = Rota::where("tecnico_id", $this->user->tecnico_id)->last("data");

                // if ($rota) {

                //     $now = Carbon::now("America/Sao_paulo");
                //     $diff = strtotime($now) - strtotime($rota->data);

                //     // SE ÚLTIMA ROTA REGISTRADA FOR MAIOR QUE 1 MINUTO
                //     if ($diff>60) {
                //         $chamado->iniciar_trajeto = true;
                //     }
    ...
    public function list()
    {
        ...
        /**
         * REMOVER POR HORA O PARAMETRO DE ORDER
         * FORÇAR A ORDENAÇÃO POR PRIORIDADE E DATA DESC
         */
        // if ($data->has('order')) {
        ...
        // } else {
            ...
            $chamados = $chamados->orderByField('ch.prioridade', ["alto", "medio", "baixo"])
                ->orderBy('ch.data', "DESC");
    ...
    public function insert()
    {
        ...
        if ($data->has("suprimentos")) {
            ...
            foreach ($suprimentos as $s) {
                ...
                ChamadoSuprimento::create(
                    [
                        'chamado_id'=>$insert->id,
                        'suprimento_id'=>$s['suprimento_id'],
                        'quantidade'=>$qtde,
                        'quantidade_enviada'=>$qtde,
                        'valor'=>$s['suprimento_valor']
                    ]
                );
            }
        }
        ...
        // Enviar Notificação
        // OneSignal::sendNotify(null, "Novo Chamado #".$insert->id, null, null, $this->auth->user()->notificacao_id);
    ...
    public function start()
    {
        ...
        // Registrar Produtividade do Técnico
        Produtividade::trigger($this->auth->auth()->uid, "iniciar", "Trajeto iniciado no chamado #".$id);
        ...
        // OneSignal::sendNotify("Chamado #".$id.": Em Trânsito", "O técnico já está à caminho.", null, null, $this->auth->user()->notificacao_id);
    ...
    public function checkin()
    {
        ...
        $data->required(["chamado_id"]);
        $data->allow(["chamado_id", "coordenadas"]);
        $data->is_int(["chamado_id"]);
        ...
        if ($configuracao->chamados_distancia_checkin) {
            ...
            if ($cliente->coordenadas) {
                ...
                $localizacao_tecnico = $data->has("coordenadas") ? $data->coordenadas : $this->auth->user()->localizacao;
        ...
        // Registrar Produtividade do Técnico
        Produtividade::trigger($this->auth->auth()->uid, "iniciar", "Checkin no chamado #".$id);

        // Enviar E-mail

        // Enviar Notificação ??
        // $cliente = Cliente::findById($chamado->cliente_id);
        // OneSignal::sendNotify("Chamado #".$id.": Checkin Realizado", "O técnico já está atendendo o chamado #".$id, null, null, $cliente->notificacao_id);
    ...
    public function pendency()
    {
         ...
         * Baixar Estoques
         */
        $local = DB::table("estoque_local")->find($this->auth->auth()->uid, "tecnico_id");
        if ($local) {
            Estoque::saida($chamado->chamado_id, $local->id);
        }
        ...
        // Registrar Produtividade do Técnico
        Produtividade::trigger($this->auth->auth()->uid, "finalizar", "Chamado em pendência #".$id);

        // Enviar E-mail

        // Enviar Notificação
        // $cliente = Cliente::findById($chamado->cliente_id);
        // OneSignal::sendNotify("Chamado #".$id.": Cancelado", "O chamado #".$id." foi cancelado", null, null, $cliente->notificacao_id);
    ...
    public function cancel()
    {
        ...
        // Inserir/Matar SLA
        Sla::trigger($id, $data->status_id, 4);

        // Registrar Produtividade do Técnico
        Produtividade::trigger($this->auth->auth()->uid, "finalizar". "Chamado #".$id." cancelado");
    ...
    public function finish()
    {
        ...
        // Registrar Produtividade do Técnico
        Produtividade::trigger($this->auth->auth()->uid, "finalizar", "Chamado #".$id." finalizado");

        // Inserir/Matar SLA
        Sla::trigger($id, 3, $up["data_solucao"]);

        // Calcular Total do SLA
        $up["sla"] = Sla::slaTotal($id);
        ...
        // Enviar Notificação
        // OneSignal::sendNotify(null, "O chamado #".$id." foi finalizado", null, null, $cliente->notificacao_id);
        // OneSignal::sendNotify(null, "Você possui novas avaliações", null, null, $cliente->notificacao_id);
    ...
    public function update($request)
    {
        ...
        if ($data->has('status_id') && $data->status_id==7) {
            ...
            // Registrar Produtividade do Técnico
            Produtividade::trigger($this->auth->auth()->uid, "finalizar", "Reagendamento do chamado #".$id);
